...
def compute_total_score(inputs: ScoreInputs, direction: str, min_score: int = 85) -> int:
    ...
    try:
        ...
        if direction == "long":
            ...
            # Short scoring logic
            resistance_distance = abs(price - inputs.r1) / price if inputs.r1 > 0 else 1
            support_distance = abs(inputs.s1 - price) / price if inputs.s1 > 0 else 1
...
async def place_bracket_trade(client, symbol: str, intents: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            # Place SL order (as per memory: limit orders, reduce-only)
            sl = intents["sl"]
            logger.debug(f"Placing SL order for {symbol}: {sl}")
            
            sl_order = await client.create_order(
                symbol=symbol,
                type="limit",  # As per memory: limit orders for SL
                side=sl["side"],
                amount=sl["quantity"],
                price=sl.get("price", sl.get("stopPrice")),  # Use price for limit order
                params={
    ...
    try:
        ...
        try:
            ...
            if signals["long"]:
                ...
                tp2_price = levels["r1"] if levels["r1"] > price else price * 1.02
                ...
                tp2_price = levels["s1"] if levels["s1"] > 0 and levels["s1"] < price else price * 0.98
